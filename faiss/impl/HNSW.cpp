...
namespace faiss
{
   ...
   // fxy_add
   HNSW::HNSW(std::vector<std::vector<int>> &metadata_multi, int M, int gamma)
       : rng(12345),
         metadata_multi(metadata_multi),
         metadata(this->empty_metadata)
   {
...
#pragma omp parallel for reduction(+ : tot_neigh) reduction(+ : tot_common) \
  ...
  reduction(+: tot_reciprocal) reduction(+: n_node)
         ...
         if (levels[i] > level)
         {
            ...
            for (size_t j = begin; j < end; j++)
            {
               if (hybrid_neighbors[j].first < 0) // mod
                  break;
               printf("%d, (%d)",
                      hybrid_neighbors[j].first,
                      hybrid_neighbors[j].second);         // mod
               neighset.insert(hybrid_neighbors[j].first); // mod
            }
            printf("]\n");
            int n_neigh = neighset.size();
   ...
   void HNSW::print_neighbor_stats(bool edge_list, bool is_hybrid) const
   {
      ...
      if (edge_list)
      {
         for (int level = 0; level <= max_level; level++)
         {
            ...
            {
               print_edges(
                   level); // this is more detailed version with edge lists
            }
         }
      }
      else
   ...
   // fxy_add
   int hnsw_binary_search(const std::vector<int> vec, int target)
   {
      // std::cout << "HNSW binary_search" << std::endl;
      int left = 0;
      ...
      while (left <= right)
      {
         ...
         if (vec[mid] == target)
         {
            // std::cout << "binary_search true" << std::endl;
            return 1; // 找到目标元素
         }
      ...
      // std::cout << "binary_search false" << std::endl;
      return 0; // 如果没有找到，返回 false
   }

   // fxy_add 检查 metadata_multi[check_v] 是否包含 required[query_idx] 的所有元素
   ...
   namespace
   {
          ...
          int level)
      {
         size_t begin, end;
         hnsw.neighbor_range(src, level, &begin, &end);
         if (hnsw.hybrid_neighbors[end - 1].first == -1)
         { // access to std::pair
            // there is enough room, find a slot to add it
            size_t i = end;
            while (i > begin)
            ...
            hnsw.hybrid_neighbors[i] =
                std::make_pair(dest, hnsw.metadata[dest]); // mod
            return;
         }
         ...
         while (resultSet.size())
         {
            hnsw.hybrid_neighbors[i++] = std::make_pair(
                resultSet.top().id, hnsw.metadata[resultSet.top().id]); // mod
            resultSet.pop();
         }
         // they may have shrunk more than just by 1 element
         while (i < end)
          ...
          std::vector<storage_idx_t> ep_per_level = {})
      {
         ...
         while (!candidates.empty())
         {
            ...
            if ((currEv.d > results.top().d && hnsw.gamma == 1) ||
                results.size() >= M)
            {
               // debug("greedy stop in construction, results size: %ld, desired
            ...
            // if (end - begin > 64) {
            //     end = begin + 64;
            //     // debug("truncating neighbor range -- M: %d, begin: %ld, new
            //     end: %ld", hnsw.M, begin, end);
            // }
            ...
            // TODO - if candidates is empty, add the next non-visited neighbor from
            // neighbors of ep or query at level l-1 only do this step if we have a
            // hybrid index (gamma > 1)
            // TODO should only backtrack if there are unvisited nodes at this level
            // debug("during BFS, gamma: %d, candidates size: %ld, results size:
            // %ld, vt.num_visited: %d, nb on level: %d, backtrack_level: %d, level:
            // %d\n", hnsw.gamma, candidates.size(), results.size(),
            // vt.num_visited(), hnsw.nb_per_level[level], backtrack_level, level);
            // if (hnsw.gamma > 1 && candidates.empty() && results.size() < M &&
            // vt.num_visited() < hnsw.nb_per_level[level] && backtrack_level <=
            // level) { if (hnsw.gamma > 1 && candidates.empty() && results.size() <
            // M && vt.num_visited() < hnsw.nb_per_level[level]) {
            //     debug("connected component exhausted too early, results size:
            //     %ld, desired size: %d, gamma = %d\n", results.size(), M,
            //     hnsw.gamma);
            ...
            //         hnsw.neighbor_range(ep_per_level[backtrack_level],
            //         backtrack_level, &begin, &end); // TODO can also do this for
            //         query instead of ep debug("\tchecking %ld neighbors of node
            //         %d at level %d\n", (end - begin), entry_point,
            //         backtrack_level); for (size_t i = begin; i < end; i++) {
            //             // debug("\t\titer %d\n", i);
            //             storage_idx_t nodeId = hnsw.neighbors[i];
          ...
          std::vector<storage_idx_t> ep_per_level = {})
      {
         ...
         while (!candidates.empty())
         {
            ...
            {
               // storage_idx_t nodeId = hnsw.neighbors[i];
               auto [nodeId, metadata] =
                   hnsw.hybrid_neighbors[i]; // storage_idx_t, int
               if (nodeId < 0)
                  break;
               if (vt.get(nodeId))
                  continue;
      ...
      // fxy_add
      int greedy_update_nearest_multi(
          const HNSW &hnsw,
          DistanceComputer &qdis,
          int level,
          storage_idx_t &nearest,
          ...
          bool dis_or_cost_in_search)
      {
         ...
         for (;;)
         {
            ...
            for (size_t i = begin; i < end; i++)
            {
               ...
               bool if_v_pass = check_metadata_all(v, aq_multi, query_id, metadata_multi);
               bool if_nearest_pass = check_metadata_all(nearest, aq_multi, query_id, metadata_multi);
               if (!if_v_pass)
               {
                  continue;
               }
               if (if_v_pass)
               {
                  ...
                  float cost;
                  if (dis_or_cost_in_search)
                     cost = qdis(v);
                  else
                     cost = all_cost[query_id][v];
                  ndis += 1;
          ...
          float &d_nearest)
      {
         ...
         for (;;)
         {
            ...
            for (size_t i = begin; i < end; i++)
            {
               ...
               // hnsw.metadata[v]); if (hnsw.metadata[v] != filter) {
               //     // debug("%s\n", "skipping vertex");
               //     continue;
               // }
               // check filter
               if (((op == EQUAL) && (hnsw.metadata[v] == filter)) ||
                   (op == OR && (hnsw.metadata[v] & filter != 0)) ||
                   (op == REGEX && CHECK_REGEX(hnsw.metadata_strings[v], regex)))
               {
                  num_found = num_found + 1;
               }
                   ...
                   (op == REGEX && CHECK_REGEX(hnsw.metadata_strings[v], regex)))
               {
                  ...
                  ndis++;
                  if (dis < d_nearest ||
                      ((op == EQUAL) && (hnsw.metadata[nearest] != filter)) ||
                      (op == OR && (hnsw.metadata[nearest] & filter == 0)) ||
                      (op == REGEX &&
                       !CHECK_REGEX(hnsw.metadata_strings[nearest], regex)))
                  {
                     nearest = v;
                     d_nearest = dis;
                  }
               ...
               for (size_t j = begin2; j < end2; j++)
               {
                  ...
                  // check filter
                  if (((op == EQUAL) && (hnsw.metadata[v2] == filter)) ||
                      (op == OR && (hnsw.metadata[v2] & filter != 0)) ||
                      (op == REGEX &&
                       CHECK_REGEX(hnsw.metadata_strings[v2], regex)))
                  {
                     num_found = num_found + 1;
                     float dis2 = qdis(v2);
                     ndis += 1;
       ...
       std::vector<storage_idx_t> ep_per_level)
   {
      ...
      // TODO check this just ignore this step if we have hybrid search, becuase
      // we need big neighbor lists
      ...
      while (!link_targets.empty())
      {
         ...
         add_link(
             *this,
             ...
             pt_id,
             other_id,
             level); // for hybrid search we also will store the metadata in
                     // the neighbor list
...
#pragma omp critical
      ...
      if (nearest < 0)
      {
         ...
         // pt_id, pt_level); std::cout << "FINISHED add_with_locks finished" <<
         // std::endl;
         return;
      }
      ...
      // printf("finished add_with_locks called for node %d, level %d\n", pt_id,
      // pt_level); std::cout << "FINISHED add_with_locks finished" << std::endl;
   }
...
#pragma omp critical
      ...
      for (; level > pt_level; level--)
      {
         // debug("--greedy update nearest at level: %d, ep: %d\n", level,
         // nearest);
         hybrid_constr_greedy_update_nearest(
             *this, ptdis, level, nearest, d_nearest); // hybrid method
         ep_per_level[level] = nearest;
      }
   ...
   namespace
   {
      ...
      // fxy_add
      int search_from_candidates_multi(
          const HNSW &hnsw,
          DistanceComputer &qdis,
          int k,
          idx_t *I,
          ...
          const SearchParametersHNSW *params = nullptr)
      {
         // std::cout << "search_from_candidates_multi" << std::endl;
         int nres = nres_in;
         int ndis = 0;
         ...
         { // candidates is heap of size max(efs, k)
            ...
            //     if (!sel || sel->is_member(v1)) {
            //         if (nres < k) {
            //             faiss::maxheap_push(++nres, C, I, d, v1);
            //         } else if (d < C[0]) {
            //             faiss::maxheap_replace_top(nres, C, I, d, v1);
            //         }
            //     }
            //     candidates.push(v1, d);
            // }
            ...
            for (size_t j = begin; j < end; j++)
            {
               ...
               if (if_bfs_filter)
               {
                  // std::cout << "if_bfs_filter" << std::endl;
         ...
         { // candidates is heap of size max(efs, k)
            ...
            for (size_t j = begin; j < end; j++)
            {
               ...
               if (v1 < 0)
                  break;
               // check filter
               if (((op == EQUAL) && (hnsw.metadata[v1] == filter)) ||
                   (op == OR && ((hnsw.metadata[v1] & filter) != 0)) ||
                   (op == REGEX &&
                    CHECK_REGEX(hnsw.metadata_strings[v1], regex)))
               {
                  num_found = num_found + 1;
               }
               if (vt.get(v1))
               ...
               // the closest of all the ones that did pass, for now just return
               // err skips vertex if it has the wrong attribute debug("looking at
               // vertex %d with metadata %d\n", v1, hnsw.metadata[v1]); if
               // (hnsw.metadata[v1] != filter) {
               //     // debug("%s\n", "skipping vertex");
               //     continue;
               // }
               ...
               for (size_t j2 = begin2; j2 < end2; j2 += 1)
               {
                  ...
                  // hnsw.metadata[v2]); std::cout << "expanding to neighbor " <<
                  // v2 << " with metadata " << hnsw.metadata[v2] << "\n" <<
                  // std::endl;
                  if (v2 < 0)
                     break;
            ...
            nstep++; // TODO - might want to only increment this if we find a
                     // neighbor that passes attr
       ...
       const SearchParametersHNSW *params) const
   {
      ...
      { // common branch
         ...
         {
            ndis_upper += greedy_update_nearest(
   ...
   HNSWStats HNSW::search_multi(
       ...
       const SearchParametersHNSW *params) const
   {
      // std::cout << "search_multi" << std::endl;
      HNSWStats stats;
      if (entry_point == -1)
      {
         return stats;
      ...
      { // common branch
         ...
         for (int level = max_level; level >= 1; level--)
         {
            ndis_upper += greedy_update_nearest_multi(
                *this,
                qdis,
                level,
                nearest,
         ...
         if (search_bounded_queue)
         { // this is the most common branch
            // std::cout << "search_bounded_queue" << std::endl;
            ...
            // search_from_candidates(
            //         *this, qdis, k, I, C, candidates, vt, stats, 0, 0,
            //         params);
            search_from_candidates_multi(
                *this,
                qdis,
                k,
                I,
         ...
         { // TODO：暂时用不到，没改
            // debug("%s\n", "reached search_bounded_queue == False");
            std::cout << "search_bounded_queue == False" << std::endl;
            ...
            while (!top_candidates.empty())
            {
               ...
               storage_idx_t label;
               std::tie(d, label) = top_candidates.top();
               faiss::maxheap_push(++nres, C, I, d, label);
               top_candidates.pop();
            }
         }
      ...
      else
      {
         ...
         std::vector<idx_t> I_to_next(candidates_size);
         std::vector<float> C_to_next(candidates_size);

         int nres = 1;
         I_to_next[0] = entry_point;
         // D_to_next[0] = qdis(entry_point);
         if (!dis_or_cost_in_search)
            C_to_next[0] = qdis(entry_point);
         else
         ...
         for (int level = max_level; level >= 0; level--)
         {
            ...
            if (level == 0)
            {
               nres = search_from_candidates_multi(
                   *this,
                   qdis,
                   k,
                   I,
      ...
      { // common branch
         ...
         for (int level = max_level; level >= 1; level--)
         {
            ndis_upper += hybrid_greedy_update_nearest(
                *this, qdis, filter, op, regex, level, nearest, d_nearest);
         }
         stats.n3 += ndis_upper;
