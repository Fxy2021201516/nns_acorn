...
{ // 输出文件名的基础部分
   ...
   for (size_t q = 0; q < coverage.size(); ++q)
   {
      ...
      // 打开文件并写入 JSON 数据
      std::cout << "save_coverage_to_json, q: " << q << std::endl;
      std::ofstream output_file(filename);
      if (output_file.is_open())
      {
         // 写入格式化后的 JSON 数据
    ...
    const std::string &folder_path)
{
   for (size_t i = 0; i < all_cost.size(); ++i)
   {
      ...
      for (const auto &pair : all_cost[i])
      {
         query_json.push_back({{"id", pair.first}, {"cost", pair.second}});
      }

      // 写入 JSON 文件
      file << query_json.dump(4); // 缩进 4 空格格式化输出
      file.close();
   }
}
...
double calculateRecall(
    const std::vector<std::vector<std::pair<int, float>>> &all_cost,
    const std::vector<faiss::idx_t> &ids, // 现在存储的是 ID 而不是 cost
    int nq,
    int k)
{
   int correct = 0;
   ...
   for (int i = 0; i < nq; ++i)
   {
      ...
      for (const int &id : query_ids)
      {
         ...
         for (int j = 0; j < k; ++j)
         {
            if (sorted_cost[j].first == id) // 仅比较 ID 是否相等
            {
               found = true;
               break;
            }
